...
### 2.3 Lakshana (Anonymous Identifier)
A **`Lakshana`** is a specialized refinement of a `DiscoveryId`. It is used for network-facing operations (sync, discovery) to prevent the Relay from clustering different graphs belonging to the same identity.

*   **Derivation:** `Lakshana = HMAC-SHA256(Branch_5_Key, "akshara.v1.discovery" || GraphId)`
*   **Purpose:** It serves as the unified "address" for both Inboxes and Data Graphs when interacting with a Relay.
*   **Opaqueness:** A Lakshana MUST be treated as an opaque token by the Relay. It provides no mathematical link to the user's `MasterSeed` or `SigningPublicKey`.
...
| :--- | :--- | :--- |
| **`BlockId`** | `0x57` | `akshara-block` (Akshara Block Base) |
| **`ManifestId`** | `0x58` | `akshara-manifest` (Akshara Manifest Base) |
