...
fn setup_test_identity_and_keys(passphrase: &str) -> (SecretIdentity, GraphId, GraphKey, String) {
    let mnemonic = SecretIdentity::generate_mnemonic().unwrap();
    let identity = SecretIdentity::from_mnemonic(&mnemonic, passphrase).unwrap();
    let graph_id = GraphId::new();
    let graph_key = identity.derive_graph_key(&graph_id).unwrap();
    (identity, graph_id, graph_key, mnemonic)
}

fn create_test_auth_manifest(identity: &SecretIdentity, id_gid: GraphId) -> (Block, Manifest) {
...
async fn create_test_leaf_and_index(
    store: &InMemoryStore,
    identity: &SecretIdentity,
    graph_id: GraphId,
    graph_key: &GraphKey,
) -> (Block, BlockId, Manifest) {
    let leaf = Block::new(
        graph_id,
        b"Target Bits".to_vec(),
    ...
    let root_index_id = builder
        .build(graph_id, store, identity, graph_key)
        .await
        .unwrap();
    ...
    (leaf, root_index_id, manifest)
}

async fn create_and_register_project(
    master: &MasterIdentity,
    relay_store: &InMemoryStore,
) -> (GraphId, GraphKey, ManifestId, Manifest, Block) {
    ...
    let id_anchor = identity_graph
        .add_resource(descriptor, true, &id_gid, &laptop_identity, &executive_pub)
        .await
        .unwrap();

    let shadow_id = laptop_identity.derive_shadow_identity(&project_id).unwrap();
    let data = Block::new(
        project_id,
    ...
    (project_id, project_key, id_anchor, manifest, data)
}
...
async fn test_sync_recursive_index_structure() {
    ...
    let (identity, graph_id, graph_key, _) = setup_test_identity_and_keys("");
    let (leaf, root_index_id, manifest) =
        create_test_leaf_and_index(&laptop_store, &identity, graph_id, &graph_key).await;
...
async fn test_identity_rebirth_bootstrap() {
    ...
    let id_gid = {
        ...
        master.identity_id().unwrap()
    };
    let laptop_identity = SecretIdentity::from_mnemonic(&mnemonic, passphrase).unwrap();
    let (id_root, id_manifest) = create_test_auth_manifest(&laptop_identity, id_gid);
    ...
    assert_eq!(derived_id_gid, id_gid);
...
async fn test_full_lifecycle_stateless_journey() {
    ...
    let (project_id, _project_key, identity_id) = {
        let master = MasterIdentity::from_mnemonic(&mnemonic, "").unwrap();
        let id_gid = master.identity_id().unwrap();
        let (p_id, p_key, _id_anchor, manifest, data) =
            create_and_register_project(&master, &relay_store).await;
        relay_store.put_block(&data).await.unwrap();
        relay_store.put_manifest(&manifest).await.unwrap();
        (p_id, p_key, id_gid)
    };

    // --- STEP 2: REBIRTH (Alice gets a new Phone) ---
    let recovered_project_key = {
        let master = MasterIdentity::from_mnemonic(&mnemonic, "").unwrap();
    ...
    let project_heads = relay_store.get_heads(&project_id).await.unwrap();
    ...
    let plaintext = block.decrypt(&project_id, &recovered_project_key).unwrap();
